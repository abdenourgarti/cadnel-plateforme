...
export default function Absences() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // Si c'est un utilisateur simple, utiliser son companyId directement
  ...
  // Récupérer les absences en fonction de l'entreprise sélectionnée dans le filtre
  useEffect(() => {
    const fetchAbsences = async () => {
      try {
        ...
        if (companyId) {
          const response = await axios.get(`http://localhost:5000/api/absence/company/${companyId}`, {
            headers: { Authorization: `Bearer ${getAuthToken()}` }
          });
          setAbsences(response.data.data);
          setFilteredAbsences(response.data.data);
        } else {
          setAbsences([]);
          setFilteredAbsences([]);
        }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        // Créer un objet FormData pour envoyer à la fois le fichier et les données
        ...
        const response = await axios.put(
          `http://localhost:5000/api/absence/${selectedAbsence.id_absence}`, 
          formData, 
          {
            headers: { 
        ...
        // Recharger les absences après la modification
        const companyId = isAdmin ? selectedEntrepriseId : currentUser.companyId;
        if (companyId) {
          const response = await axios.get(`http://localhost:5000/api/absence/company/${companyId}`, {
            headers: { Authorization: `Bearer ${getAuthToken()}` }
  ...
  // Charger les employés pour les utilisateurs simples (non-admin)
  useEffect(() => {
    if (!isAdmin && currentUser.companyId) {
      ...
      const fetchEmployes = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/api/employe/${currentUser.companyId}`, {
            headers: { Authorization: `Bearer ${getAuthToken()}` }
          });
  ...
  const exportFormik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await axios.post('http://localhost:5000/api/absence/export', payload, {
          headers: { Authorization: `Bearer ${getAuthToken()}` },
  ...
  // Récupérer les employés en fonction de l'entreprise sélectionnée dans le formulaire de recherche
  useEffect(() => {
    if (isAdmin && rechercheFormik.values.entrepriseId) {
      const fetchEmployes = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/api/employe/${rechercheFormik.values.entrepriseId}`, {
            headers: { Authorization: `Bearer ${getAuthToken()}` }
          });
          setEmployes(response.data.data);
        } catch (error) {
  ...
  }, [rechercheFormik.values.entrepriseId, isAdmin]);
  ...
  const applyFilters = (absenceList, filters = rechercheFormik.values) => {
    ...
    if (filters.entrepriseId) {
      filtered = filtered.filter(absence => absence.id_company === parseInt(filters.entrepriseId));
    }
    
    if (filters.employeId) {
      filtered = filtered.filter(absence => absence.id_employe === parseInt(filters.employeId));
    }
  ...
  const handleEdit = (absence) => {
    ...
    formik.setValues({
      status: Boolean(absence.status), // Convertir 0/1 en true/false
      motif: absence.motif || '',
  ...
  const handleDownload = (documentFilename) => {  // Renommez le paramètre pour éviter le conflit
    ...
    axios({
      ...
      .then((response) => {
        ...
        const link = document.createElement('a');  // Maintenant 'document' fait référence à l'objet global
        link.href = url;
        link.setAttribute('download', documentFilename);  // Utiliser le paramètre renommé
  ...
  const handleOpenExportModal = () => {
    ...
    // Préremplir l'entreprise pour les utilisateurs non-admin
    if (!isAdmin && currentUser.companyId) {
      exportFormik.setFieldValue('entrepriseId', currentUser.companyId);
    }
  };
  ...
  const handleFileChange = (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      // Juste mettre à jour l'affichage pour l'utilisateur
      formik.setFieldValue('document', file.name);
      // Le fichier réel sera récupéré depuis l'input au moment de l'envoi
    }
  };
    ...
    <MainLayout>
      <div className="p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          {/* Afficher le bouton d'export pour les admins avec entreprise sélectionnée OU pour les non-admins */}
          {((isAdmin && selectedEntrepriseId) || !isAdmin) && (
            <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  disabled={isAdmin && !rechercheFormik.values.entrepriseId} // Désactivé seulement pour admin sans entreprise sélectionnée
                  className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-emerald-500 focus:border-emerald-500"
                >
              ...
              {filteredAbsences.length > 0 ? (
                filteredAbsences.map((absence) => (
                  <tr key={absence.id_absence} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{absence?.employe?.nomcomplet}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{absence?.employe?.departement?.nom}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{absence?.employe?.zone?.nom}</td>
